...
if (isset($_SESSION['go_id']) && isset($_SESSION['return_id'])) {
    ...
    $db = DataBase::getPdo();
    $statement = $db->prepare('SELECT price, flight_number FROM flights WHERE flights.id = :go_id');
    $statement->bindValue(':go_id', $go_id, PDO::PARAM_INT);
    $statement->execute();
    $go_flight = $statement->fetch(PDO::FETCH_ASSOC);
    $go_price = $go_flight['price'];

    $statement = $db->prepare('SELECT price, flight_number FROM flights WHERE flights.id = :return_id');
